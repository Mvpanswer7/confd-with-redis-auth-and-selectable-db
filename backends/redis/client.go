...
func tryConnect(machines []string, password string, db int) (redis.Conn, error) {
	var err error
	for _, address := range machines {
		...
		if db != 0 {
			dialops = append(dialops, redis.DialDatabase(db))
		}
...
func (c *Client) connectedClient() (redis.Conn, error) {
	...
	if c.client == nil {
		var err error
		c.client, err = tryConnect(c.machines, c.password, c.db)
		if err != nil {
			return nil, err
		}
	}
...
func NewRedisClient(machines []string, password string, db int) (*Client, error) {
	var err error
	clientWrapper := &Client{machines: machines, password: password, db: db, client: nil}
	clientWrapper.client, err = tryConnect(machines, password, db)
